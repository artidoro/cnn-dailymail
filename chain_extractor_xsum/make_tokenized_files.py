...
def get_art_abs(story_file):
    # This function reads a story file and emits article and abstract.

    # Read article lines + summary lines
    ...
    article = ' <split1> '.join(article_lines) #THIS  IS WHERE IT HAPPENS.
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("USAGE: python make_tokenized_files.py <data_dir>")
        sys.exit()
    data_dir = sys.argv[1]
    # cnn_stories_dir = sys.argv[1]
    # dm_stories_dir = sys.argv[2]

    # ARTI: These are the file names.
    # test.document
    # test.summary
    # train.document
    # train.summary
    # validation.document
    # validation.summary
    test_article_lines = open(data_dir + "/test.document").readlines().replace("\n", "")
    test_abstract_lines = open(data_dir + "/test.summary").readlines().replace("\n", "")
    val_article_lines = open(data_dir + "/validation.document").readlines().replace("\n", "")
    val_abstract_lines = open(data_dir + "/validation.summary").readlines().replace("\n", "")
    train_article_lines = open(data_dir + "/train.document").readlines().replace("\n", "")
    train_abstract_lines = open(data_dir + "/train.summary").readlines().replace("\n", "")

    # TODO: ARTI add the checks back
    ...
    # ARTI: Our data is already tokenized and already has <s> </s> and <split1> tokens.
    # Run stanford tokenizer on both stories dirs, outputting to tokenized stories directories
    # tokenize_stories(cnn_stories_dir, cnn_tokenized_stories_dir)
    # tokenize_stories(dm_stories_dir, dm_tokenized_stories_dir)

    # ARTI: all we need to do is read the train, test, val file and write them to bin.
